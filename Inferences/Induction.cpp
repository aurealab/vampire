 ...
 *
 * ?y : L[y] &_{con_i} ( y = con_i(..dec(y)..) -> subterm(dec(y),y))
             & (!x : subterm(x,y) -> subterm(destructor(x),y))
             & !z : subterm(z,y) => ~L[z]
 *
 * i.e. we use the subterm predicate of the term algebra
 *
 */
void InductionClauseIterator::performStructInductionThree(Clause* premise, Literal* origLit, Literal* lit, Term* term, InferenceRule rule) 
{
  ...
  // get subterm predicate
  unsigned st = ta->getSubtermPredicate();

  // make ( y = con_i(..dec(y)..) -> subterm(dec(y),y))  for each constructor
  ...
  for(unsigned i=0;i<ta->nConstructors();i++){
    ...
    if(con->recursive()){
      ...
      // create subterm(cons(x1,..,xn),y)
      Formula* subterm_coni = new AtomicFormula(Literal::create2(st,true,
                                TermList(Term::create(con->functor(),(unsigned)varTerms.size(),varTerms.begin())),
                                y));

      FormulaList* subterms = 0;
      Stack<unsigned>::Iterator vtit(ta_vars);
      while(vtit.hasNext()){
        FormulaList::push(new AtomicFormula(Literal::create2(st,true,TermList(vtit.next(),false),y)),subterms);
      }
      ASS(subterms);
      Formula* ax = Formula::quantify(new BinaryFormula(Connective::IMP,subterm_coni,
                      JunctionFormula::generalJunction(Connective::AND,subterms)));

      // now create a conjunction of subterm(d(y),y) for each d
      FormulaList* And = FormulaList::empty(); 
      Stack<TermList>::Iterator tit(taTerms);
      while(tit.hasNext()){
        Formula* f = new AtomicFormula(Literal::create2(st,true,tit.next(),y));
        FormulaList::push(f,And);
      }
      ASS(And);
      Formula* imp = new BinaryFormula(Connective::IMP,
  ...
  // now !z : subterm(z,y) => ~L[z]
  TermReplacement cr2(term,z);
  Formula* subtermImpNL = Formula::quantify(new BinaryFormula(Connective::IMP,
                            new AtomicFormula(Literal::create2(st,true,z,y)),
                            new AtomicFormula(cr2.transform(clit))));

  FormulaList::push(subtermImpNL,conjunction);
  Formula* exists = new QuantifiedFormula(Connective::EXISTS, VList::singleton(y.var()),0,
